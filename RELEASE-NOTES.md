...
- A blocked update now names every conflicting host, record, DHCP Reservation,
...
- UI and documentation now consistently distinguish a DHCP Reservation, a
...
A correctness release, plus the work needed to keep category blocking usable as upstream feeds keep growing. Several rules that the appliance enforces in more than one place turned out to disagree with themselves, and the disagreements were doing real damage: an address with what is now called an IP Reservation could stay flagged as a rogue device indefinitely, an address could silently lose its hostname while its DNS record still pointed at it, and the updater could proceed with its disk-space check and all four of its upgrade guards quietly disabled. Separately, the Malware blocklist category had stopped refreshing entirely because the feed outgrew a hardcoded download limit. Schema migrates forward to **version 54**. No breaking API changes.

### New
- **Service accounts with API tokens.** Settings > Access > Users can now create an account of type **Service account**, for a machine rather than a person. It has no password and is refused at the login endpoint outright; it authenticates only by presenting an API token as `Authorization: Bearer cidr_pat_...`. Tokens are issued per account from the same screen, carry that account's role and nothing more, and are **shown once at creation**: only a SHA-256 hash is stored, so the database never holds a working credential. Each token can be revoked individually without disturbing the account or its other tokens, and the list shows when each was last used. An expiry of **0 days means never**, which is the right default for an unattended poller that has nobody to renew it; any other value sets a hard expiry. The account's role is re-read on every request, so a role change or a deletion takes effect immediately rather than at the next token issue.
- **Maximum feed size is now an operator setting.** Settings > Filtering > Categories gains a "Max Feed Size (MB)" field, default **128**, range 1 to 2048. It caps a single category download, so a category pointed at a runaway or hostile source URL still cannot fill the disk. The old ceiling was hardcoded at 20MB, which is what made the Malware category fail (see Fixed).

### Changed
- **Category blocking scales to feeds of millions of domains.** Upstream lists outgrew the previous design: the Block List Project malware feed passed 51MB and 2.65 million entries in 2026, up from around 5MB for most other categories. Three things changed. Feeds are **streamed and parsed line by line** rather than buffered whole, so importing one no longer holds a 51MB string plus two multi-million-element arrays in memory at once, and peak memory during a refresh dropped from roughly 790MB to under 200MB. Domains are **written in batches** with the event loop released between them, so a refresh no longer freezes DNS for about four seconds in a single transaction; the longest uninterrupted pause is now around 60ms. And domain lookups **read from SQLite instead of an in-memory map**, which removes a hard ceiling at 16,777,216 domains where the resolver would have started throwing, drops several hundred MB of permanent memory, and makes allowed-domain edits and category toggles instant because there is no longer a map to rebuild on every change (that rebuild had grown to about 2.8 seconds). A lookup costs roughly 4 microseconds including the subdomain walk, against a hot path that already reads SQLite per query.
- **Blocklist refreshes skip unchanged feeds.** Each category now records the feed's `ETag` and `Last-Modified` and sends them back as a conditional request, so a scheduled refresh of an unchanged feed completes in milliseconds with no download, no parse and no database writes. Feeds are also requested compressed, which takes the malware list from 51MB to 17MB on the wire. Previously every scheduled refresh re-downloaded every enabled feed in full.
- **A refresh can no longer leave a gap in blocking.** The old refresh deleted a category's domains and then re-inserted them inside one transaction. Domains are now staged first, applied to the live table before anything is removed, and only then swept, so a domain present in both the old and new feed is never momentarily absent. A refresh that fails partway leaves the previous list completely untouched instead of a partial one, and a feed that downloads but parses to zero valid domains is refused rather than being allowed to empty the category.
...
### Fixed
- **The Malware category can be refreshed again.** It failed with "Response too large" and stayed empty. Category downloads were capped at a hardcoded 20MB and the Block List Project malware feed had grown to 51MB, so the guard was doing its job against a limit that upstream had outgrown. Raising the number alone would not have been enough: the old code buffered the whole feed into a single string, which JavaScript refuses to do beyond 512MB, and loaded every domain into a map that fails hard past 16.7 million entries. Both limits are gone (see Changed), and the remaining ceiling is the operator-configurable feed size, defaulting to 128MB. Any category whose feed exceeded the old limit was affected; malware is the one that actually crossed it. The error message now names the feed's actual size, the current limit and the setting to raise, instead of a bare "Response too large".
- **An address with an IP Reservation or static assignment could stay flagged as a rogue device forever.** Whether an address counts as claimed was decided in two places, and they used different rules. The scanner honored the administrative declaration, the passive DNS watcher did not. Creating an IP Reservation does not clear an existing rogue flag, so if an address was flagged first, only a scan would clear it, and on a subnet the scanner does not cover no scan ever comes. The address list showed rogue ahead of the legacy locked state, so the row read as ROGUE indefinitely. Both paths now ask the same question.
- **An address could lose its hostname while its DNS record still pointed at it.** A startup task removes hostnames whose DNS record has gone away. It rebuilt the expected name by joining the record name to its zone, but names arriving from DHCP Leases and DHCP Reservations were stored exactly as the device reported them, so a device calling itself `S24-Ultra` produced a name that never matched the lowercased form the rest of the system uses. The record was present and correct, and the hostname was stripped anyway. Storing one canonical form (see Changed) removes the mismatch rather than papering over it at each comparison.
...
- **The MAC vendor database download had no size limit and no timeout.** It fetched the Wireshark manufacturer file and read the whole response into memory in one piece, with nothing bounding either how large that response could be or how long the request could hang. A broken or hostile upstream could therefore stall the refresh indefinitely or hand back an unbounded body. It now goes through the same guarded download path the blocklist feeds use, with a 30 second timeout and a 32MB ceiling against a real file of roughly 5MB, and it is read a line at a time rather than buffered whole. The address that file is fetched from has not changed and was never operator-controlled, so this was never a redirection risk.
- **Searching the blocked-domain list was slow on large categories.** Every search ran two full passes over the domain table, one purely to produce a result count, and with the malware category at over 2.6 million entries that count alone blocked the server for up to 400ms per search. The count is gone: the search now fetches one row beyond the page to decide whether a Next button should be live. A search that matches something is 70 to 400 times faster; a search that matches nothing is about twice as fast, because it still has to read the whole list to be sure. **The results pager now shows "Page 3" rather than "Page 3 of 12"**, since an exact match count cannot be produced without the pass that was removed.
- **A Pi-hole import could give one address a second hostname, which the DNS page refuses.** The DNS page has always rejected pointing a second name at an address already named by a DHCP Reservation, an active lease, or another A record, and tells you to add a CNAME instead. The importer skipped that check, so a file could quietly create what the UI forbids. It is now applied to imports as well. An import's own records are not judged against each other, so a file that deliberately maps one address to two names still imports, and re-importing the same file still works.
- **A Pi-hole import could create a CNAME that the DNS page refuses.** The importer only checked that a CNAME's target looked like a valid domain, so a file could land a CNAME in a local zone pointing at an arbitrary external host, or at a name that does not exist anywhere. The DNS page has always required a CNAME target to be fully qualified, inside the zone it is being written to, and already present there as an enabled A or CNAME record. Imports now apply the same three rules. A file's own records count as present, so a CNAME whose target is defined further down the same file still imports.
- **A failed import now reports every problem at once, and imports nothing.** Validation used to stop at the first bad record, so fixing a file meant re-uploading once per problem. The response now lists every record it could not accept, with the reason for each, and a structured `problems` array alongside the message. Nothing is written when anything is wrong: a partial import leaves you guessing which half landed.
- **The password rule was enforced at first-run setup but not when changing a password.** Creating the initial admin required an uppercase letter, a lowercase letter and a number. Changing a password afterwards checked only the length, so the policy could be escaped seconds after install by setting something like `aaaaaaaa`. The first-run wizard also checked only the length client-side, so a password it appeared to accept was rejected only after submit. All three now apply one rule, and the wizard fetches that rule from the server and shows it under the field rather than restating it.
- **An update could roll back a healthy install after you changed the web port.** The updater worked out which port to health-check after switching slots by reading the database through the `sqlite3` command-line tool, which the installer has never installed. That lookup therefore always failed silently and the updater probed 8443 regardless of what was configured. If the port had been changed in the UI to anything other than 8443 or 443, the post-switch health check could not reach the appliance, judged the new version broken, and rolled back a slot that was working. It now reads the setting with the bundled Node runtime, the same way the rest of the update does.
- **A failed update could leave the appliance down instead of rolling itself back.** The A/B design exists so a bad release is undone automatically, and there were two holes in that. First, if the new version failed to *start* at all, rather than starting and then failing its health check, the updater aborted at the restart itself and never reached the health verification step that triggers the automatic rollback, so the appliance was left pointing at the broken version. That left an odd asymmetry: a new version that came up unhealthy was rolled back correctly, while one that could not start at all, the more likely failure, was not. Second, a version that cannot start is restarted repeatedly by the service manager, and that quickly exhausts a rate limit which applies to the *service* rather than to the version, so it went on to block the rollback from starting the known-good version being restored. The updater now treats a failed restart as a health failure and rolls back through the normal path, and clears that rate limit before restoring. `cidrella-rollback` clears it too, since it is run precisely when a version is crash-looping. DNS and DHCP were never affected either way: dnsmasq is a separate service that updates never restart.
- **An address with a manual A record could show as available.** In the subnet address list, an address that had a DNS record but no stored row of its own was built as a placeholder that declared itself to have no static DNS, which suppressed the lookup that would have found the record. The same address seen through the DHCP scope view showed correctly. Both now agree.
- **The scanner status chip could show three different states at once.** With a scan queued but not yet started, the chip label read "Scanner pending", the tooltip on that same button read "Scanner Active", and the status dot showed OK. Pending is now a state of its own and all three agree.
- **Host details showed a different hostname than the row you clicked.** The address table strips the subnet's own domain from a hostname, so `laptop.home.lan` displays as `laptop`. The host details dialog showed the full name, so the same device appeared under two names one click apart.
- **A subnet with no name rendered as "10.0.0.0/24 — undefined"** in the network and VLAN settings pickers.
- **The DHCP scope dialog accepted a pool whose start was after its end** and submitted it, where the network wizard caught the same mistake with a clear message. It is now rejected before submit, with the reason.
- **The footer's last-sync time had no days bucket**, so twelve hours after a sync it read "30h AGO" rather than "1d ago", and an unreadable timestamp rendered "NaNh AGO".
- **Domain fields accepted values that are not domains.** The shared name check only required a value to start and end with a letter or digit with letters, digits, dots and hyphens in between. That let through three shapes DNS has no meaning for: an IP address such as `10.0.0.1`, a name with an empty label such as `sub..evil.com`, and a label longer than the 63 characters DNS permits. The consequence worth naming is the first one: a dotted-quad typed into a subnet or scope domain name was written into the generated dnsmasq configuration as though it were a name. All fifteen places that validate a domain now apply one rule and reject all three. Deliberately unchanged: a single-label name such as `lan` or `local` is still accepted, because `domain=lan` is a legitimate configuration that existing installs use, and mixed case is still accepted because DNS is case-insensitive and names are already normalized when stored.
...
- **A subnet could be given VLAN ID 0, which is not an assignable VLAN.** 802.1Q reserves 0 for priority tagging, so the usable range is 1 to 4094, and that is what the VLAN table and every VLAN form already enforced. Subnet create, update and divide checked 0 to 4094 instead, so a subnet could be stamped with a VLAN ID that no VLAN record is allowed to exist for, and two such subnets then reported colliding with each other on it. All of them now share one check. The UI never offered 0, so reaching this needed a direct API call, and any existing row is left as it is until you next save the subnet.

### Upgrade notes
- **Schema migrates forward to version 53**, normalizing stored DNS names (`052`) and reworking blocklist storage for large feeds (`053`). Both are forward-only, and `052` is idempotent. Existing zone names, subnet domain pointers and record names are rewritten together, so nothing is left pointing at the old spelling.
- **Blocked-domain lists are re-downloaded once after upgrading.** Migration `053` rebuilds the domain table in a more compact form (roughly half the disk per domain, and faster to search) and recreates it empty rather than copying millions of rows through the upgrade, which would have stalled the first boot. The table holds nothing but data fetched from upstream feeds, so the scheduler simply refetches every enabled category about ten seconds after the service starts. **Category blocking is inactive for those first few seconds**, and for as long as the downloads take on a slow link. Your allowed-domains list is operator data and is not touched. The migration itself is quick: measured at 0.17s against a database holding 2.65 million domains.
- **Database file size after this release.** On a fresh install expect roughly 155MB once the malware category has downloaded: about 90MB of domains and about 65MB of working space that each refresh reuses rather than growing. On an upgraded appliance the file does not shrink, because SQLite returns freed pages to its own free list instead of returning them to the filesystem. A box that was holding the old 2.65-million-domain table stays at its previous size (around 317MB in testing) and then reuses that space for subsequent refreshes, so it stops growing rather than getting smaller. Reclaiming it needs a manual `VACUUM`, which is not run automatically because it rewrites the whole database and needs roughly double the space while it works.
- **A stored domain that was never valid is now refused when you next edit it.** The tighter check above runs on write, not on existing rows, so nothing breaks on upgrade and no stored value is rewritten. If an appliance is holding a domain field containing an IP address, an empty label or an over-long label, that record keeps working exactly as before until someone saves it, at which point the save is rejected with the reason. Such a value was already producing a dnsmasq directive that meant nothing, so the rejection is surfacing a pre-existing problem rather than creating one.
...
- **Dynamic DHCP lease hostnames now stay in DNS.** Lease sync now persists the effective hostname used for DHCP-sourced A records, including generated fallbacks, so later DHCP config regeneration no longer prunes dynamic lease DNS entries. DHCP Reservations still take priority over dynamic lease hostnames for the same IP.
- **IP table Type rendering is consistent.** Networks, DHCP, and DNS now share the same Type labels and pill colors for `static DNS`, `dynamic DHCP`, `DHCP Reservation`, `IP Reservation`, and `rogue`, with the shared rendering logic centralized for future column customization work.
- **DHCP scopes show the full address range.** Selecting a DHCP Scope now shows every IP in that scope, including available/unassigned addresses, with DHCP Leases and DHCP Reservations overlaid on top. Unassigned addresses remain `Status = available` with no Type.
...
- **Duplicate stale DHCP host rows are reconciled.** Startup and DHCP Lease sync now keep only one `ip_addresses` row per DHCP MAC, preferring active DHCP Lease or DHCP Reservation rows and otherwise keeping the most recently seen row. Older offline duplicates from previous leases no longer linger in Networks.
- **Old offline DHCP host memories expire after 24 hours.** DHCP-owned `ip_addresses` rows with no active DHCP Lease or DHCP Reservation are now pruned once they have been offline for more than 24 hours, returning those IPs to an available state instead of keeping stale host/MAC details indefinitely.
- **Scan-only offline rows are no longer retained.** When active scans re-check an `available` IP with no DNS hostname, DHCP backing, DHCP Reservation, IP Reservation, or scan override and it does not respond, CIDRella now deletes the ephemeral `ip_addresses` row instead of keeping an old `last_seen_at` record forever.
...
- **Grid view readability.** Sub-pixel rounding artifacts fixed by replacing container `gap` with per-cell `box-shadow`. Every 16th column gets a thicker inset shadow for easier visual counting. DHCP Reservations render dark blue and DNS-configured IPs pale green (distinct from IP Reservations in violet, gateway orange, and system-range gray). Tooltip now includes a `Role: network` / `Role: broadcast` line for boundary cells (their Type stays `system`). Legend always shows every possible color plus any user-defined range types.
- **Multi-select bulk IP actions on the grid.** Right-click a rectangle of selected cells to create or release IP Reservations, remove addresses from a DHCP Scope, or add a DHCP Scope. Middle-of-pool removal is refused client-side with a guidance toast ("can't split a DHCP pool") instead of hitting the server.
- **Safe-divide with lossy-IP cleanup.** Dividing a subnet that contains DHCP Reservations, DNS A records, leases, or IP Reservations that would fall outside the new child prefixes now opens a confirmation dialog listing each flagged IP with its reason (`network` / `broadcast` / `outside_selection`), carrier (DHCP Reservation / IP record / DNS A), hostname, and MAC. "Divide Anyway" retries with `force_lossy: true`, and the server returns a `lossy_cleanup` summary that the client surfaces as a toast ("Removed 3 DHCP Reservations, 2 DNS records, 1 lease"). `force` and `force_lossy` are separate gates so the two failure modes stay distinguishable.
...
- **DNS-in-pool warn toast.** Saving a DNS A record whose IP is inside an active DHCP pool fires a warn toast ("DHCP may reassign this address. Consider a DHCP Reservation instead"). The record still saves.
- **Ranges table cleanup and IP Reservation ranges.** Ranges now hides auto-generated system rows (Network / Broadcast / Gateway stay in the grid but not the table) and inserts contiguous IP Reservation ranges so a held span shows as one row instead of N.
...
- **PTR lookup by zone name, not subnet.** `findPtrLocation` in `utils/ip-sync.js` no longer filters by `subnet_id`. DHCP Reservations on any subnet now write PTRs into whichever reverse zone covers the IP, regardless of which subnet nominally owns the zone. This makes shared-zone and cross-subnet PTR flows work correctly.
- **DHCP Reservation POST guard.** `/api/dhcp/reservations` now rejects requests against subnets that are non-leaf or unallocated, closing a divide-then-insert race that could strand a DHCP Reservation on an about-to-be-deleted parent.
...
- **DHCP Scope / DHCP Reservation dropdowns filter to allocated leaf subnets only.** Non-leaf and unallocated subnets no longer appear as pick targets in `ScopeDialog` or the DhcpPanel DHCP Reservation dialog, matching the server-side guard.
...
- **Lossy-divide detector missing non-boundary carriers.** Previously only caught gateway/boundary IPs. Now also catches DHCP Reservations, `ip_addresses` rows with real state including IP Reservations, DNS A records, and `outside_selection` for partial divides where one or more children don't cover an existing record.
