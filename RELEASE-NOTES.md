...
A feature release on top of the v0.4.15 resilience base: encrypted DNS forwarding, DNSSEC validation, rogue-DHCP detection, passive device/OS fingerprinting, an authoritative-only mode, and a consolidated DNS-forwarding settings UI. Schema migrates forward to **version 50**. No breaking API changes. **Hosts must be on v0.4.15 to upgrade directly.** Older hosts upgrade to v0.4.15 first (see Upgrade notes).
...
- **Runtime and dependency refresh.** Bundled Node updated to 24.18.0 (security line). Cleared npm audit advisories: axios and form-data (client, high) and body-parser (server, low). Major upgrades: better-sqlite3 13 (prebuilt binding layout; the updater and build now accept it), @duckdb/node-api 1.5.5, PrimeVue 5 with @primeuix/themes 3 and primeicons 8, and Pinia 4. Routine minors: Vite 8.1.5, Vue 3.5.40, Vue Router 5.2, helmet, express-rate-limit.
...
- **Schema migrates forward to version 50**, adding the rogue-DHCP authorized-server allowlist (`047`), the per-MAC `device_fingerprints` table (`049`), and the GeoIP IP/CIDR allowlist (`050`). The `048` migration slot is intentionally skipped: it was an interim GeoIP-whitelist table that was superseded by unifying the whitelist into the existing one. Transparent and forward-only.
