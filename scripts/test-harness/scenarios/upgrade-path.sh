...
scenario_setup() {
  ...
  local tries=0
  until curl -skf https://127.0.0.1:8443/api/health >/dev/null 2>&1; do
    tries=$((tries + 1))
    [ "$tries" -ge 30 ] && { echo "FROM release never became healthy" >&2; return 1; }
    sleep 2
  done

  # Remember the active slot so the assert phase can prove it switched.
...
scenario_assert() {
  ...
  assert_command_ok "grep -q '\"state\"[[:space:]]*:[[:space:]]*\"success\"' /var/lib/cidrella/update-status.json || grep -q 'success' /var/lib/cidrella/update-status.json"

  # ─── A/B slot actually switched ───────────────────────
  assert_command_ok "! diff -q /tmp/slot-before.txt /tmp/slot-after.txt"

  # ─── Candidate version is what's running ──────────────
  assert_command_ok "grep -q '\"version\": \"${CANDIDATE_VERSION%%-*}' /opt/cidrella/package.json"

  # ─── Deep health on the new slot ──────────────────────
  ...
  # ─── Schema migrated to the candidate's version ───────
  assert_command_ok "[ \"\$(sqlite3 /var/lib/cidrella/cidrella.db 'SELECT MAX(version) FROM schema_version')\" = \"$EXPECTED_SCHEMA\" ]"

  # ─── Services up, switchover recorded ─────────────────
  assert_systemctl_active cidrella
  assert_systemctl_active cidrella-dnsmasq
  assert_file_contains /var/lib/cidrella/events.jsonl "switch"
}
