...
// already-classified device is logged to device_fingerprint_changes.
export function upsertFingerprint(db, fp) {
  ...
  // the strongest automatic classification, so a value that lost either of
  // those comparisons never landed and is not drift.
  if (before) logDrift(db, mac, before, getByMac(db, mac));
  return result;
}
