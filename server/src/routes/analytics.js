  ...
  return async (req, res) => {
    try {
      ...
      if (!VALID_RANGES.includes(range)) {
        return res
          .status(400)
          .json({ error: `Invalid range. Must be one of: ${VALID_RANGES.join(', ')}` });
      }
...
router.get(
  '/dnssec/top-unsupported-domains',
  auth,
  analyticsRoute(queryTopDomainsWithoutDnssec, { defaultLimit: '10' }),
);
...
router.get(
  '/blocklist/top-domains',
  auth,
  analyticsRoute(queryTopDomainsByAction, { fixedAction: 'blocked_blocklist', defaultLimit: '10' }),
);

// GET /api/analytics/blocklist/top-categories?range=24h&limit=10
router.get(
  '/blocklist/top-categories',
  auth,
  analyticsRoute(queryTopBlockReasons, { fixedAction: 'blocked_blocklist', defaultLimit: '10' }),
);

// GET /api/analytics/blocklist/top-client-domains?range=24h&limit=20
router.get(
  '/blocklist/top-client-domains',
  auth,
  analyticsRoute(queryTopClientDomainPairsByAction, {
    enrich: true,
    fixedAction: 'blocked_blocklist',
    defaultLimit: '20',
  }),
