-- Remove the legacy available/assigned/locked/dhcp storage vocabulary.
-- allocation_state is now the only allocation authority. Rebuild both the
-- parent and event tables so lifecycle history keeps its original row ids.
