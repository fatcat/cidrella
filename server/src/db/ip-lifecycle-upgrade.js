...
function loadFacts(db, { localAddresses = localIpv4Set() } = {}) {
  ...
  const reservations = db.prepare('SELECT * FROM dhcp_reservations WHERE enabled = 1').all();
  const leases = db
    .prepare(
      `
  ...
  `,
    ...
    .all()
    .map((record) => ({ ...record, subnet: bestSubnet(subnets, record.ip_address) }))
  ...
  const reservationByKey = new Map(
    reservations.map((row) => [identityKey(row.subnet_id, row.ip_address), row]),
  );
  const leaseByKey = new Map(
    leases.map((row) => [identityKey(row.subnet_id, row.ip_address), row]),
  );
  ...
  const inEnabledPool = (subnetId, ip) =>
    scopes.some(
      (scope) =>
        scope.enabled === 1 &&
  ...
  const protectedKind = (subnetId, ip) => {
    ...
    if (
      canonical === canonicalizeIp(subnet.network_address) ||
...
function canNameProtectedAddress(facts, subnetId, ip) {
  ...
  const canonical = canonicalizeIp(ip) || ip;
  return Boolean(subnet && canonical === canonicalizeIp(subnet.gateway_address));
}
...
export function inventoryLegacyIpLifecycle(db, options = {}) {
  ...
  const addIssue = (category, subnetId, ip, reason, remediation, affectedResources = []) => {
    ...
    if (existing) {
      existing.affected_resources = [
        ...new Set([...existing.affected_resources, ...affectedResources]),
      ];
  ...
  for (const record of facts.dns) {
    ...
    const matchingScopes = facts.scopes.filter(
      (scope) =>
        scope.enabled === 1 &&
        scope.subnet_id === subnetId &&
        containsAddress(scope.start_ip, scope.end_ip, ip),
    ...
    if (matchingScopes.length) {
      addIssue(
        'manual_dns_inside_dynamic_pool',
        subnetId,
        ip,
    ...
    if (facts.protectedKind(subnetId, ip) && !canNameProtectedAddress(facts, subnetId, ip)) {
      const protectedKind = facts.protectedKind(subnetId, ip);
      addIssue(
        'protocol_claim_on_protected_address',
        subnetId,
        ip,
    ...
    if (!canNameProtectedAddress(facts, subnetId, ip) && facts.reservationByKey.has(key)) {
    ...
    } else if (!canNameProtectedAddress(facts, subnetId, ip) && facts.leaseByKey.has(key)) {
      const lease = facts.leaseByKey.get(key);
      addIssue(
        'competing_dns_and_dynamic_lease',
        subnetId,
        ip,
  ...
  for (const lease of facts.leases) {
    ...
    if (!reservation && !facts.inEnabledPool(lease.subnet_id, lease.ip_address)) {
      addIssue(
        'dynamic_lease_outside_enabled_pool',
        lease.subnet_id,
        lease.ip_address,
    ...
    if (
      ...
      addIssue(
        'reservation_lease_client_mismatch',
        lease.subnet_id,
        lease.ip_address,
  ...
  for (const row of facts.ips) {
    ...
    if (
      row.status === 'locked' &&
      !facts.protectedKind(row.subnet_id, row.ip_address) &&
  ...
  const assignedWithoutManualDns = db
    .prepare(
      `
  ...
  const disabledDnsLooksActive = db
    .prepare(
      `
  ...
  `,
    ...
    .get().count;
  const disabledReservationLooksActive = db
    .prepare(
      `
    SELECT COUNT(DISTINCT ip.id) AS count
    FROM ip_addresses ip
  ...
  const claimKeys = new Set([
    ...facts.ipByKey.keys(),
    ...facts.reservationByKey.keys(),
    ...facts.leaseByKey.keys(),
    ...facts.dnsByKey.keys(),
  ]);
  ...
  for (const key of claimKeys) {
    ...
    if (
      row?.status === 'locked' &&
      !facts.protectedKind(subnetId, ip) &&
      !isLegacyLocalDnsAllocation(facts, row, key)
    )
      claims.add('admin_reservation');
  ...
  for (const row of facts.ips) {
    ...
    if (
      ...
      !expectedNames.some((name) => hostnameMatches(row.hostname, name))
    )
      hostnameDisagreements++;
    const expectedMacs = [
    ...
    if (
      row.mac_address &&
      expectedMacs.length &&
      !expectedMacs.includes(String(row.mac_address).toLowerCase())
    )
      macDisagreements++;
  }
  ...
  return {
    ...
    summary: {
      ...
      ips_with_multiple_static_dns_names: [...facts.dnsByKey.values()].filter(
      ...
      locked_addresses_inside_enabled_pool: facts.ips.filter(
        (row) => row.status === 'locked' && facts.inEnabledPool(row.subnet_id, row.ip_address),
      ).length,
      ...
      disabled_configuration_active_looking:
        disabledDnsLooksActive + disabledReservationLooksActive,
      ...
      duplicate_canonical_identity_groups: [...facts.ipByKey.values()].filter(
        (rows) => rows.length > 1,
      ).length,
      duplicate_ip_rows_across_subnets: [...canonicalIpSubnets.values()].filter(
        (ids) => ids.size > 1,
      ).length,
      noncanonical_address_rows: facts.ips.filter(
        (row) => canonicalizeIp(row.ip_address) !== row.ip_address,
      ).length,
      ipv4_mapped_address_rows: facts.ips.filter((row) =>
        row.ip_address.toLowerCase().includes('::ffff:'),
      ).length,
      unscoped_ipv6_link_local_rows: facts.ips.filter(
        (row) => isLinkLocalV6(row.ip_address) && !row.interface_id,
      ).length,
      hostname_disagreements: hostnameDisagreements,
      mac_disagreements: macDisagreements,
    },
    conflicts: [...issues.values()].sort(
      (left, right) =>
