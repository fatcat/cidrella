...
async function previewAndDivide(parentId, body) {
  ...
  const result = await request(app)
    .post(`/api/subnets/${parentId}/divide`)
    .send({
      ...body,
      force: true,
...
describe('canonical cross-model network transformations', () => {
  it('previews and creates default-sized pools for every child of a scoped network', async () => {
    ...
    const sourceScope = getDb()
      .prepare('SELECT id, lease_time FROM dhcp_scopes WHERE subnet_id = ?')
      .get(parent.id);
    ...
    expect(preview.status, JSON.stringify(preview.body)).toBe(200);
    expect(
      preview.body.plan.targets.map((target) => ({
        cidr: target.cidr,
    ...
    expect(result.status, JSON.stringify(result.body)).toBe(200);
    expect(
      poolRows(result.body.children.map((child) => child.id)).map((pool) => [
        pool.start_ip,
        pool.end_ip,
        pool.lease_time,
      ]),
    ...
    expect(result.body.pool_adjustments).toContainEqual(
    ...
    const lowerScope = getDb()
      .prepare('SELECT id FROM dhcp_scopes WHERE subnet_id = ?')
      .get(children[0].id);
    const lowerRangeIds = getDb()
      .prepare('SELECT range_id FROM dhcp_scope_pools WHERE scope_id = ?')
      .all(lowerScope.id)
      .map((row) => row.range_id);
    getDb().transaction(() => {
    ...
    expect(poolRows([parent.id]).map((pool) => [pool.start_ip, pool.end_ip])).toEqual([
      ['192.0.2.33', '192.0.2.64'],
    ]);
    expect(
      getDb()
        .prepare(
          `SELECT ip_address, is_online, is_rogue, rogue_reason
  ...
  it('creates four default pools and preserves gateways, leases, reservations, and custom ranges', async () => {
    ...
    expect(
      ...
      expectedIpv4Split('172.30.0.0/24', 26, 'last').map((child) => [
        child.cidr,
        'last',
        child.gateway,
      ]),
    ...
    expect(
      topology.filter((row) => row.allocation_state === 'gateway').map((row) => row.ip_address),
    ).toEqual(['172.30.0.126', '172.30.0.190', '172.30.0.254', '172.30.0.62']);

    expect(
      poolRows(children.map((child) => child.id)).map((row) => [
        row.start_ip,
        row.end_ip,
        row.router,
      ]),
    ...
    expect(
      db.prepare("SELECT subnet_id FROM dhcp_leases WHERE hostname = 'leased-host'").get()
        .subnet_id,
    ).toBe(children[2].id);
    expect(
      db.prepare("SELECT subnet_id FROM dhcp_reservations WHERE hostname = 'reserved-host'").get()
        .subnet_id,
    ).toBe(children[3].id);
    const customRanges = db
      .prepare(
        `
  ...
  it('uses default pool sizing for both an unequal carve and its merge', async () => {
    ...
    const before = poolRows(children.map((child) => child.id)).map((row) => [
      row.start_ip,
      row.end_ip,
    ]);
    ...
    expect(merged.status, JSON.stringify(merged.body)).toBe(200);
    expect(poolRows([parent.id]).map((row) => [row.start_ip, row.end_ip])).toEqual([
      ['172.31.0.33', '172.31.0.64'],
    ]);
  ...
  it('blocks merge when child scope policies differ', async () => {
    ...
    getDb()
      .prepare(`UPDATE dhcp_scopes SET lease_time = '8h' WHERE subnet_id = ?`)
      .run(children[1].id);
    ...
    expect(merge.body.conflicts).toContainEqual(
      expect.objectContaining({
        code: 'dhcp_scope_policy_conflict',
      }),
    );
  ...
  it('makes direct and repeated subdivision semantically equivalent', async () => {
    ...
    expect(relativeSemantics(direct.cidr, leafRows(direct.id))).toEqual(
      relativeSemantics(repeated.cidr, leafRows(repeated.id)),
    );
  });
  ...
  it('rolls back every affected table when scope projection fails mid-transaction', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) AS count FROM dhcp_scope_pools').get().count).toBe(
      before.pools,
    );
