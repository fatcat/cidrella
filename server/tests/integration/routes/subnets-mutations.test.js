...
describe('PUT /api/subnets/:id, domain_name pointer semantics', () => {
  ...
  it('allows two subnets to share a forward zone via matching domain_name', async () => {
    const a = await mkSubnet({
      cidr: '10.31.0.0/24',
      name: 'A',
      status: 'allocated',
      gateway_address: '10.31.0.1',
    });
    const b = await mkSubnet({
      cidr: '10.31.1.0/24',
      name: 'B',
      status: 'allocated',
      gateway_address: '10.31.1.1',
    });
  ...
  it('clearing domain_name leaves the zone in place (other subnets may need it)', async () => {
    const s = await mkSubnet({
      ...
      name: 'clearer',
      status: 'allocated',
      gateway_address: '10.33.0.1',
    });
    await configure(s.id, {
      name: 'clearer',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'still-here.test',
    });
...
describe('DNS zone CRUD ↔ subnets.domain_name sync', () => {
  it('PUT /api/dns/zones/:id rename updates every subnet whose domain_name matched', async () => {
    const a = await mkSubnet({
      ...
      name: 'zone-put-a',
      status: 'allocated',
      gateway_address: '10.35.0.1',
    });
    const b = await mkSubnet({
      cidr: '10.35.1.0/24',
      name: 'zone-put-b',
      status: 'allocated',
      gateway_address: '10.35.1.1',
    });
    await configure(a.id, {
      name: 'zone-put-a',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'before.test',
    });
  ...
  it('DELETE /api/dns/zones/:id clears every subnet that pointed at it', async () => {
    const a = await mkSubnet({
      ...
      name: 'zone-del-a',
      status: 'allocated',
      gateway_address: '10.36.0.1',
    });
    const b = await mkSubnet({
      cidr: '10.36.1.0/24',
      name: 'zone-del-b',
      status: 'allocated',
      gateway_address: '10.36.1.1',
    });
    await configure(a.id, {
      name: 'zone-del-a',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'gone.test',
    });
...
describe('DNS address metadata sync', () => {
  it('projects shared IP metadata through Networks, DNS, and DHCP reads', async () => {
    ...
    await configure(s.id, {
      ...
      dhcp_start_ip: '10.129.0.2',
      dhcp_end_ip: '10.129.0.5',
    ...
    const fingerprint = {
      ...
      dhcp_fingerprint_hostname: 'DESKTOP-ROUTES',
  ...
  it('projects reverse DNS rows through the same canonical IP read model', async () => {
    const s = await mkSubnet({
      cidr: '10.89.0.0/29',
      name: 'reverse-read-model',
      status: 'allocated',
      gateway_address: '10.89.0.1',
    });
    await configure(s.id, {
      name: 'reverse-read-model',
      create_reverse_dns: true,
  ...
  it('renaming a DNS A record updates the ip_addresses row hostname', async () => {
    const s = await mkSubnet({
      ...
      name: 'dns-rename',
      status: 'allocated',
      gateway_address: '10.80.0.1',
    });
    await configure(s.id, {
      name: 'dns-rename',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'dns-rename.test',
    });
    ...
    const rename = await request(app)
      .put(`/api/dns/zones/${zone.id}/records/${create.body.id}`)
      .send({
        name: 'host-v2',
        type: 'A',
        value: '10.80.0.50',
  ...
  it('rejects a second A hostname for the same IP and directs aliases to CNAME', async () => {
    const s = await mkSubnet({
      cidr: '10.84.0.0/24',
      name: 'dns-single-hostname',
      status: 'allocated',
      gateway_address: '10.84.0.1',
    });
    await configure(s.id, {
      name: 'dns-single-hostname',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'single-hostname.test',
    });
    ...
    const cname = await request(app).post(`/api/dns/zones/${zone.id}/records`).send({
      name: 'secondary',
      type: 'CNAME',
      value: 'primary.single-hostname.test',
    });
  ...
  it('does not allow reservation-sourced DNS records to be edited or deleted manually', async () => {
    const s = await mkSubnet({
      cidr: '10.85.0.0/24',
      name: 'dns-derived-records',
      status: 'allocated',
      gateway_address: '10.85.0.1',
    });
    await configure(s.id, {
      name: 'dns-derived-records',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'derived-records.test',
    });
...
describe('PUT /api/subnets/:id, structural guards', () => {
  ...
  it('rejects CIDR change in the body when the value differs (R2 #5)', async () => {
    ...
    const ok = await request(app)
      .put(`/api/subnets/${s.id}`)
      .send({ cidr: s.cidr, name: 'renamed' });
  ...
  it('refuses a gateway change that lands inside an existing DHCP pool (R3 #2)', async () => {
    const s = await mkSubnet({
      ...
      name: 'gw-pool',
      status: 'allocated',
      gateway_address: '10.41.0.1',
    });
    await configure(s.id, {
      name: 'gw-pool',
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.41.0.100',
      dhcp_end_ip: '10.41.0.200',
    });

    // Inside the pool → 409.
    const bad = await request(app)
      .put(`/api/subnets/${s.id}`)
      .send({ gateway_address: '10.41.0.150' });
    ...
    const ok = await request(app)
      .put(`/api/subnets/${s.id}`)
      .send({ gateway_address: '10.41.0.2' });
    expect(ok.status).toBe(200);
  });
});
...
describe('PUT /api/dhcp/scopes/:id, pool resize guard (R4 #4)', () => {
  it('refuses a pool resize that would include the subnet gateway', async () => {
    ...
    await configure(s.id, {
      ...
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.42.0.100',
      dhcp_end_ip: '10.42.0.200',
    });
    ...
    const bad = await request(app)
      ...
      .send({ start_ip: '10.42.0.1', end_ip: '10.42.0.200' });
    expect(bad.status).toBe(409);

    // A valid resize still works.
    const ok = await request(app)
      .put(`/api/dhcp/scopes/${scope.id}`)
      .send({ start_ip: '10.42.0.50', end_ip: '10.42.0.200' });
    expect(ok.status).toBe(200);
  });
});
...
describe('gateway-in-pool invariant holds on every route that writes a pool', () => {
  it('POST /api/subnets/:id/configure refuses an explicit pool containing the gateway', async () => {
    const s = await mkSubnet({
      ...
      name: 'cfg-gw',
      status: 'allocated',
      gateway_address: '10.111.0.1',
    });
    const bad = await request(app).post(`/api/subnets/${s.id}/configure`).send({
      name: 'cfg-gw',
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.111.0.1',
      dhcp_end_ip: '10.111.0.200',
    });
    expect(bad.status).toBe(409);
  ...
  it('POST /api/subnets/:id/configure refuses a gateway strictly inside the pool', async () => {
    ...
    const bad = await request(app).post(`/api/subnets/${s.id}/configure`).send({
      ...
      create_dhcp_scope: true,
      gateway_address: '10.114.0.150',
      dhcp_start_ip: '10.114.0.100',
      dhcp_end_ip: '10.114.0.200',
    });
    expect(bad.status).toBe(409);
  });
  ...
  it('PUT /api/subnets/:subnetId/ranges/:id refuses editing a live pool onto the gateway', async () => {
    const s = await mkSubnet({
      ...
      name: 'rng-put-gw',
      status: 'allocated',
      gateway_address: '10.113.0.1',
    });
    await configure(s.id, {
      name: 'rng-put-gw',
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.113.0.100',
      dhcp_end_ip: '10.113.0.200',
    });
...
describe('folder_id is validated identically on every route that accepts it', () => {
  async function realFolder() {
    const res = await request(app)
      .post('/api/folders')
      .send({ name: `F${Date.now()}${Math.floor(process.hrtime()[1] % 1000)}` });
    expect(res.status).toBe(201);
    return res.body.id;
  }
...
describe('GET /api/dhcp/scopes/:id/addresses, lifecycle state', () => {
  it('includes ip_addresses lifecycle state for unassigned addresses in the scope', async () => {
    ...
    await configure(s.id, {
      ...
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.44.0.100',
      dhcp_end_ip: '10.44.0.110',
    });
  ...
  it('shows retained lease history as expired without occupying a scope address', async () => {
    ...
    await configure(s.id, {
      ...
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.45.0.100',
      dhcp_end_ip: '10.45.0.110',
    });
    const scopes = await request(app).get('/api/dhcp/scopes');
    const scope = scopes.body.find((sc) => sc.subnet_id === s.id);
    const { getDb } = await import('../../../src/db/init.js');
    ...
    const history = leases.body.find(
      (row) => row.subnet_id === s.id && row.ip_address === '10.45.0.104',
    );
...
describe('canonical IP allocation endpoints', () => {
  ...
  it('bulk allocation skips protected topology addresses', async () => {
    const s = await mkSubnet({
      cidr: '10.47.0.0/24',
      name: 'canonical-bulk-allocation',
      status: 'allocated',
...
describe('DELETE /api/subnets/:subnetId/ranges/:id, scope guard', () => {
  ...
  it('refuses to delete a range that has a DHCP scope attached', async () => {
    const s = await mkSubnet({
      ...
      name: 'range-del',
      status: 'allocated',
      gateway_address: '10.43.0.1',
    });
    await configure(s.id, {
      name: 'range-del',
      create_reverse_dns: false,
      create_dhcp_scope: true,
      dhcp_start_ip: '10.43.0.100',
      dhcp_end_ip: '10.43.0.200',
    });
