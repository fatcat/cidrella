...
describe('POST /api/subnets/:id/divide, data preservation', () => {
  it('requires and applies explicit gateway policies for custom-gateway children', async () => {
    ...
    await configure(parent.id, {
      name: 'Custom gateway split',
      gateway_policy: 'custom',
      gateway_address: '192.0.3.100',
    ...
    expect(
      result.body.children.map((child) => [
        child.cidr,
        child.gateway_policy,
        child.gateway_address,
      ]),
    ).toEqual([
  ...
  it('canonically creates default scopes and transfers gateways, boundaries, and leases across a two-way split', async () => {
    ...
    await configure(parent.id, {
      ...
      dhcp_start_ip: '192.0.2.20',
      dhcp_end_ip: '192.0.2.240',
    });
    db.prepare(
      `
      INSERT INTO dhcp_leases (subnet_id, ip_address, mac_address, hostname, expires_at)
    ...
    expect(
      children.map((child) => [child.cidr, child.gateway_policy, child.gateway_address]),
    ).toEqual([
    ...
    expect(
      scopes
        .sort((a, b) => ipToLong(a.start_ip) - ipToLong(b.start_ip))
        .map((scope) => [scope.start_ip, scope.end_ip, scope.router]),
    ).toEqual([
      ['192.0.2.17', '192.0.2.32', '192.0.2.126'],
      ['192.0.2.145', '192.0.2.160', '192.0.2.254'],
    ]);
    expect(
      db.prepare("SELECT subnet_id FROM dhcp_leases WHERE hostname = 'split-client'").get()
        .subnet_id,
    ).toBe(children[0].id);
    ...
    expect(
      db
        .prepare(
          "SELECT allocation_state FROM ip_addresses WHERE subnet_id = ? AND ip_address = '192.0.2.254'",
        )
        .get(parent.id).allocation_state,
    ).toBe('gateway');
    expect(
      db
        .prepare(
          "SELECT allocation_state FROM ip_addresses WHERE subnet_id = ? AND ip_address = '192.0.2.126'",
        )
        .get(parent.id).allocation_state,
    ).toBe('unassigned');
    expect(
      db.prepare('SELECT COUNT(*) AS count FROM dhcp_scopes WHERE subnet_id = ?').get(parent.id)
        .count,
    ).toBe(1);
    expect(
      db
    ...
    `,
        ...
        .get(parent.id),
    ).toEqual({ start_ip: '192.0.2.33', end_ip: '192.0.2.64' });
    expect(
      db.prepare("SELECT subnet_id FROM dhcp_leases WHERE hostname = 'split-client'").get()
        .subnet_id,
    ).toBe(parent.id);
  ...
  it('transfers DHCP Reservations into the child that contains the IP', async () => {
    ...
    expect(
      upperResv.body.find((r) => r.ip_address === '10.10.1.50' && r.hostname === 'carrier'),
    ).toBeDefined();
  });
  ...
  it('forward DNS zone and its records survive the divide (post-decouple: zone is subnet-agnostic)', async () => {
    ...
    await request(app)
      .post(`/api/dns/zones/${fwd.id}/records`)
      .send({ name: 'survivor', type: 'A', value: '10.11.0.50' });
...
describe('POST /api/subnets/:id/divide, lossy gate', () => {
  ...
  it('requires exact conflict resolutions when lossy rows exist', async () => {
    ...
    await request(app).post('/api/dhcp/reservations').send({
      subnet_id: parent.id,
      ip_address: '10.13.1.255',
      mac_address: 'aa:bb:cc:00:00:21',
      hostname: 'doomed',
    });
  ...
  it('rejects partial or unrelated conflict acknowledgements without deleting anything', async () => {
    ...
    const reservation = await request(app).post('/api/dhcp/reservations').send({
      subnet_id: parent.id,
      ip_address: '10.24.1.255',
      mac_address: 'aa:bb:cc:00:24:01',
      hostname: 'boundary-client',
    });
    ...
    expect(
      getDb().prepare('SELECT id FROM dhcp_reservations WHERE id = ?').get(reservation.body.id),
    ).toBeDefined();
  ...
  it('flags DNS A records landing on a boundary IP', async () => {
    const parent = await createSubnet({
      ...
      name: 'Lossy-dns',
      status: 'allocated',
      gateway_address: '10.15.0.1',
    });
    await configure(parent.id, {
      name: 'Lossy-dns',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'lossy-dns.test',
    });
    ...
    await request(app)
      .post(`/api/dns/zones/${fwd.id}/records`)
      .send({ name: 'edge', type: 'A', value: '10.15.0.255' });
  ...
  it('finds boundary records in other manual zones without including shared-zone outsiders', async () => {
    ...
    await configure(parent.id, {
      name: 'Shared-zone-source',
      create_reverse_dns: false,
      create_dhcp_scope: false,
    });
    const zone = await request(app).post('/api/dns/zones').send({
      name: 'shared-boundaries.test',
      type: 'forward',
    });
    expect(zone.status).toBe(201);
    expect(
      (
        await request(app).post(`/api/dns/zones/${zone.body.id}/records`).send({
          name: 'inside',
          type: 'A',
          value: '10.25.0.255',
        })
    ...
    expect(preview.body.lossy).toEqual(
  ...
  it('preserves rows in explicit remainder children when partially selecting a divide', async () => {
    ...
    const res = await divide(parent.id, {
      new_prefix: 23,
      force: true,
      selected_cidrs: ['10.16.0.0/23'],
    });
    expect(res.status).toBe(200);
    const remainder = res.body.children.find((child) => child.cidr === '10.16.2.0/23');
    expect(remainder).toBeDefined();
    expect(
      getDb().prepare("SELECT subnet_id FROM ip_addresses WHERE ip_address = '10.16.3.50'").get()
        .subnet_id,
    ).toBe(remainder.id);
  });
...
describe('POST /api/subnets/:id/divide, reviewed conflict cleanup', () => {
  it('deletes DHCP Reservations, DNS A records, and reports counts', async () => {
    const parent = await createSubnet({
      ...
      name: 'LossyCleanup',
      status: 'allocated',
      gateway_address: '10.19.0.1',
    });
    await configure(parent.id, {
      name: 'LossyCleanup',
      create_reverse_dns: false,
      create_dhcp_scope: false,
      domain_name: 'lossy-cleanup.test',
    });
    ...
    const resvRes = await request(app).post('/api/dhcp/reservations').send({
      subnet_id: parent.id,
      ip_address: '10.19.1.255',
      mac_address: 'aa:bb:cc:00:77:01',
      hostname: 'doomed',
    });
...
describe('POST /api/subnets/:id/divide, gateway/pool conflict handling', () => {
  it('shrinks the pool to exclude the child gateway and reports the adjustment', async () => {
    ...
    await configure(parent.id, {
      ...
      dhcp_start_ip: '10.18.0.2',
      dhcp_end_ip: '10.18.3.254',
    });
