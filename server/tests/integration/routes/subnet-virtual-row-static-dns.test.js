...
describe('canonical allocation has no protocol fallback', () => {
  it('leaves an unpersisted address unassigned even if a raw A record exists', async () => {
    ...
    const zoneId = db
      .prepare("INSERT INTO dns_zones (name, type, enabled) VALUES ('audit.lan','forward',1)")
      .run().lastInsertRowid;
    db.prepare(
    ...
    db.prepare('DELETE FROM ip_addresses WHERE subnet_id = ? AND ip_address IN (?, ?)').run(
      subnetId,
      '10.77.0.99',
      '10.77.0.98',
    );
    const persisted = db
      .prepare('SELECT COUNT(*) FROM ip_addresses WHERE subnet_id = ? AND ip_address = ?')
      .pluck()
      .get(subnetId, '10.77.0.99');
