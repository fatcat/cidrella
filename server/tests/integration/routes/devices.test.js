...
beforeEach(() =>
  db.exec('DELETE FROM device_fingerprints; DELETE FROM device_fingerprint_changes;'),
);
...
describe('GET /api/devices/:mac/fingerprint/history', () => {
  it('reports device_type/os_family/vendor_class drift, newest first', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.map((c) => c.field).sort()).toEqual([
      'device_type',
      'os_family',
      'vendor_class',
    ]);
...
describe('PUT /api/devices/:mac/fingerprint (override)', () => {
  it('sets a manual override that a later dhcp capture cannot clobber', async () => {
    const put = await request(app)
      .put('/api/devices/aa:bb:cc:dd:ee:ff/fingerprint')
      .send({ device_type: 'Printer', os_family: 'Linux' });
