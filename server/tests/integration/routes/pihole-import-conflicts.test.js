...
describe('#18: a conflicting A record is rejected and named', () => {
  it('refuses a second name for an address that already has one', async () => {
    db.prepare(
      "INSERT INTO dns_records (zone_id, name, type, value, enabled, source) VALUES (?, 'nas', 'A', '10.9.0.5', 1, 'manual')",
    ).run(zoneId);
  ...
  it('names the DHCP Reservation when that is the source of the clash', async () => {
    db.prepare(
      "INSERT INTO subnets (cidr, name, prefix_length, network_address, broadcast_address, total_addresses, status, depth, domain_name) VALUES ('10.9.0.0/24','s',24,'10.9.0.0','10.9.0.255',256,'allocated',0,'audit.lan')",
    ).run();
    const subnetId = db.prepare("SELECT id FROM subnets WHERE cidr='10.9.0.0/24'").pluck().get();
    db.prepare(
      "INSERT INTO dhcp_reservations (subnet_id, mac_address, ip_address, hostname, enabled) VALUES (?, 'aa:bb:cc:dd:ee:ff', '10.9.0.7', 'printer', 1)",
    ).run(subnetId);
...
describe('#18: duplicate A records inside one import', () => {
  it('rejects the whole import and reports every A record sharing the IP', async () => {
    ...
    expect(res.body.problems.map((problem) => problem.name).sort()).toEqual(['host-a', 'host-b']);
...
describe('Pi-hole import lifecycle ownership', () => {
  it('rolls back the whole import when an existing PTR names another zone', async () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM dns_records WHERE type = 'A'").get()).toEqual({
      count: 0,
    });
    expect(
      db
        .prepare('SELECT value FROM dns_records WHERE zone_id = ? AND name = ?')
        .get(reverseZoneId, '43'),
    ).toEqual({ value: 'legacy.other.test' });
  });
  ...
  it('promotes the managed PTR placeholder to the imported DNS hostname', async () => {
    ...
    `,
        ...
        .get(reverseZoneId),
    ).toEqual({ value: 'imported.audit.lan', source: 'dns' });
    expect(
      db
        .prepare(
          `
