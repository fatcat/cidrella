...
describe('POST /api/geoip/allowlist', () => {
  it('accepts an IPv4 CIDR and lists it', async () => {
    const res = await request(app)
      ...
      .send({ value: '203.0.113.0/24', reason: 'partner range' });
  ...
  it('accepts an IPv6 CIDR', async () => {
    expect(
      (await request(app).post('/api/geoip/allowlist').send({ value: '2001:db8::/32' })).status,
    ).toBe(201);
  });

  it('rejects an invalid IP/CIDR', async () => {
    expect(
      (await request(app).post('/api/geoip/allowlist').send({ value: '10.0.0.0/33' })).status,
    ).toBe(400);
    expect((await request(app).post('/api/geoip/allowlist').send({ value: 'nope' })).status).toBe(
      400,
    );
  });
  ...
  it('409s on a duplicate', async () => {
    ...
    expect(
      (await request(app).post('/api/geoip/allowlist').send({ value: '8.8.8.8' })).status,
    ).toBe(409);
  });
});
...
describe('POST /api/geoip/allowlist: canonicalization (v0.4.16)', () => {
  ...
  it('409s on a different spelling of the same network', async () => {
    ...
    expect(
      (await request(app).post('/api/geoip/allowlist').send({ value: '010.0.0.0/8' })).status,
    ).toBe(409);
    expect(
      (await request(app).post('/api/geoip/allowlist').send({ value: '10.99.0.1/8' })).status,
    ).toBe(409);
  });
  ...
  it('canonicalizes IPv6 case and compression before dedup', async () => {
    const first = await request(app)
      .post('/api/geoip/allowlist')
      .send({ value: '2001:DB8:0:0:0:0:0:0/32' });
