...
describe('mutually exclusive DNS and DHCP allocation', () => {
  it('rejects DNS inside an enabled dynamic pool without creating a record', async () => {
    ...
    expect(
      db.prepare("SELECT * FROM ip_addresses WHERE ip_address = '10.120.0.50'").get(),
    ).toBeUndefined();
  });
  ...
  it('rejects DNS over a hostname-less DHCP Reservation', async () => {
    ...
    expect(
      db
        .prepare("SELECT allocation_state FROM ip_addresses WHERE ip_address = '10.120.0.50'")
        .get(),
  ...
  it('rejects static DHCP over DNS without creating a reservation', async () => {
    ...
    expect(
      db
        .prepare("SELECT allocation_state FROM ip_addresses WHERE ip_address = '10.120.0.51'")
        .get(),
  ...
  it('activates a reserved address through DNS or static DHCP', async () => {
    ...
    expect((await postReservation(subnetId, '10.120.0.53', 'aa:bb:cc:dd:ee:53')).status).toBe(201);

    expect(
      db
        .prepare("SELECT allocation_state FROM ip_addresses WHERE ip_address = '10.120.0.52'")
        .get(),
    ).toEqual({ allocation_state: 'static_dns' });
    expect(
      db
        .prepare("SELECT allocation_state FROM ip_addresses WHERE ip_address = '10.120.0.53'")
        .get(),
    ).toEqual({ allocation_state: 'static_dhcp' });
  });
  ...
  it('allows only one of concurrent DNS and static DHCP claims to commit', async () => {
    ...
    expect([dnsResponse.status, dhcpResponse.status].filter((status) => status < 300)).toHaveLength(
      1,
    );
    ...
    expect(
      db.prepare("SELECT allocation_state FROM ip_addresses WHERE ip_address = '10.120.0.54'").get()
        .allocation_state,
    ).toMatch(/^(static_dns|static_dhcp)$/);
  });
});
...
describe('scope conflicts in both operation orders', () => {
  ...
  it('rejects resizing an enabled scope over DNS and preserves old bounds', async () => {
    ...
    expect(response.status).toBe(409);
    expect(db.prepare('SELECT start_ip, end_ip FROM ranges WHERE id = ?').get(rangeId)).toEqual({
      start_ip: '10.120.0.20',
      end_ip: '10.120.0.40',
    });
  });
});
...
describe('protected topology addresses', () => {
  it('allows a DNS name for the gateway without transferring allocation ownership', async () => {
    ...
    const removed = await request(app).delete(
      `/api/dns/zones/${zoneId}/records/${created.body.id}`,
    );
