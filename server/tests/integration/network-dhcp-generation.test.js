...
describe('canonical DHCP generated sink', () => {
  it('emits every configured interval, preserves holes, and excludes IP Reservations', () => {
    const { subnetId, scopeId } = createScope({
      ...
      pools: [
        ['10.230.0.20', '10.230.0.61'],
        ['10.230.0.65', '10.230.0.240'],
      ],
      leaseTime: '12h',
    });
    db.prepare(
    ...
    expect(content).toContain(
      `dhcp-range=set:scope${scopeId},10.230.0.20,10.230.0.61,255.255.255.0,12h`,
    );
    expect(content).toContain(
      `dhcp-range=set:scope${scopeId},10.230.0.65,10.230.0.69,255.255.255.0,12h`,
    );
    expect(content).toContain(
      `dhcp-range=set:scope${scopeId},10.230.0.71,10.230.0.240,255.255.255.0,12h`,
    );
