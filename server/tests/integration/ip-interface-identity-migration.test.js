...
describe('migrations 057-059 canonical IP identity', () => {
  it('preserves addresses and events while removing legacy status storage', async () => {
    ...
    expect(
      db.prepare("SELECT * FROM ip_addresses WHERE ip_address = '10.77.0.42'").get(),
    ).toMatchObject({
      id: addressId,
    ...
    expect(
      db.prepare('SELECT * FROM ip_events WHERE ip_address_id = ?').get(addressId),
    ).toMatchObject({
      subnet_id: subnetId,
    ...
    expect(db.prepare('SELECT MAX(version) AS version FROM schema_version').get().version).toBe(69);
    expect(
      db
        .prepare('PRAGMA table_info(ip_addresses)')
        .all()
    ...
    expect(
      db
        .prepare(
          "SELECT interface_id FROM ip_addresses WHERE ip_address = 'fe80::42' ORDER BY interface_id",
        )
        .all(),
