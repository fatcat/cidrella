...
- Installs that have already been rooted (attacker code has already run locally, so the integrity of the install itself is compromised, no key rotation helps).
...
2. On the printout, clearly label:
   ...
   - Passphrase hint (not the passphrase itself, just something only you can decode)
...
- Immediately after using the break-glass for a primary rotation, if your threat model considers any use of the key a use-once event (most threat models don't require this, since the break-glass is *designed* to be reusable, but some stricter ones do)
