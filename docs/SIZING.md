...
Of the CIDRella budget, the Node process itself holds ~150 MB baseline (Vue dev/build chunk, express, better-sqlite3, lru-cache, MaxMind GeoLite2 DB, etc.). Everything else is workload-driven: SQLite row cache, DuckDB analytics ring buffer, DNS-proxy in-flight map, and any large allocated subnets.
