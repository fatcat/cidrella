...
<template>
  <div class="blocklist-search" style="display: flex; flex-direction: column; height: 100%">
    ...
    <DataTable
      v-if="searchResults.items.length > 0 || searchPerformed"
      :value="searchResults.items"
      stripedRows
      size="small"
      :paginator="searchResults.items.length > 256"
      :rows="256"
    ...
    >
      <template #empty>
        <EmptyState
          ...
          description="Try a different search. Matches include every enabled category."
        />
      </template>
...
async function doSearch(fromPagination = false) {
  ...
  try {
    searchResults.value = await store.searchDomains(
