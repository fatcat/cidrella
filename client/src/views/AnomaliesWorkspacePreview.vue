  ...
  <div class="anomaly-preview" data-track="anomalies-workspace-preview">
    <header class="preview-banner">
      <div>
        ...
        <p v-if="mode === 'sample'">
          The detector reports {{ SAMPLE_STATUS.flaggedWindows }} flagged windows. Six of them are
          worth your evening. This surface separates <b>unusual for this device</b> from
          <b>shaped like an attack</b>, then shows the DNS traffic that earned the flag.
        </p>
        <p v-else>
          Your own detector, in the new layout. The queue, the score history and the evidence table
          are live. Threat shape and peer medians are not, because nothing computes them yet.
        </p>
      </div>
      <div class="preview-banner-actions">
        <div class="mode-switch" role="group" aria-label="Data source">
          <button
            ...
            data-track="anomaly-preview-mode"
    ...
    <section class="status-rail" aria-label="Detector status">
      ...
      <div>
        ...
        <span class="v"
          >{{ rail.monitored }} <small>/ {{ rail.learning }} learning</small></span
        >
      </div>
      <div>
    ...
    <div class="triage-row">
      <section class="panel queue-panel" aria-label="Triage queue">
        ...
        <div class="queue-filters">
          <button
            ...
            data-track="anomaly-preview-filter"
        ...
        <div class="queue-state" v-if="mode === 'live' && liveState === 'loading'">
          Loading your detector data...
        </div>
        <div class="queue-state error" v-else-if="mode === 'live' && liveState === 'error'">
          {{ liveError }}
        </div>
        <div class="queue-state" v-else-if="!queueRows.length">No devices match this filter.</div>
        <div class="queue" v-else>
          ...
          >
            <button
              ...
              data-track="anomaly-preview-queue-row"
              ...
              <span class="row-main">
                ...
                <span class="row-tags" v-if="row.tags.length">
                  <span
                    ...
                    :key="tag"
                    class="row-tag"
                    :class="{ hot: index === 0 && row.band === 'critical' }"
                    >{{ tag }}</span
                  >
                </span>
              </span>
              <span class="row-right">
                <span class="row-urgency" :style="{ color: BAND_COLORS[row.band] }">{{
                  row.scoreLabel
                }}</span>
                <svg
                  v-if="row.spark.length > 1"
                  class="row-spark"
                  viewBox="0 0 76 20"
                  width="76"
                  height="20"
            ...
            <button
              type="button"
              class="row-detail"
              data-track="anomaly-preview-row-detail"
              :aria-label="`Show detail for ${row.name}`"
        ...
        <div class="map-body">
          ...
          >
            ...
            <g v-for="tick in MAP_TICKS" :key="tick">
              ...
              <text
                :x="mapX(tick)"
                :y="MAP.h - MAP.pad.b + 16"
                text-anchor="middle"
                class="axis-num"
              >
                {{ tick }}
              </text>
              <text :x="MAP.pad.l - 9" :y="mapY(tick) + 3" text-anchor="end" class="axis-num">
                {{ tick }}
              </text>
            </g>
            <text
            ...
            <text :x="mapX(98)" :y="mapY(96)" text-anchor="end" class="quad-label hot">
              INVESTIGATE
            </text>
            <text :x="mapX(2)" :y="mapY(96)" class="quad-label">Suspicious, steady</text>
            <text :x="mapX(98)" :y="mapY(2)" text-anchor="end" class="quad-label">
              Just unusual
            </text>
            ...
            <circle
              ...
              :key="point.id"
              class="map-dot"
              :class="{ selected: point.selected, actionable: point.actionable }"
              :cx="point.cx"
              :cy="point.cy"
              :r="point.r"
              :fill="point.color"
              :fill-opacity="point.actionable ? 0.85 : 0.45"
              :stroke="point.ringed ? point.color : 'transparent'"
              stroke-width="1.5"
              stroke-opacity="0.55"
              data-track="anomaly-preview-map-dot"
              @click="selectDevice(point.id)"
            >
              <title>{{ point.title }}</title>
            </circle>
            <text :x="leadLabel.x" :y="leadLabel.y" text-anchor="end" class="lead-label">
              {{ leadLabel.text }}
            </text>
          </svg>
          ...
          <div class="map-legend">
            ...
            <p class="legend-foot">
              Dot size is query volume in the window. A ring means the device was flagged on 3 or
              more of the last 24 windows.
            </p>
          </div>
        </div>
      </section>
      ...
      <section class="panel missing-axis" v-else>
        ...
        <p>
          The triage map is sample only. Its vertical axis is a threat shape score over entropy,
          NXDOMAIN rate, name length, subdomain depth and block rate, and nothing computes that yet.
          Switch back to Sample to see the shape of it.
        </p>
      </section>
    </div>
      ...
      >
        <div class="detail-nav">
          <button
            ...
            class="nav-button"
            :disabled="!hasPrev"
            data-track="anomaly-preview-detail-prev"
            @click="stepDetail(-1)"
          >
          ...
          <button
            ...
            class="nav-button"
            :disabled="!hasNext"
            data-track="anomaly-preview-detail-next"
            @click="stepDetail(1)"
          >
            Next <i class="pi pi-chevron-right" aria-hidden="true" />
          </button>
          <span class="detail-pos" v-if="detailIndex >= 0"
            >{{ detailIndex + 1 }} of {{ queueRows.length }}</span
          >
          <button
            type="button"
            class="nav-close"
            aria-label="Close detail"
            data-track="anomaly-preview-detail-close"
            @click="closeDetail"
          >
        ...
        <div class="entity-body">
          <div class="queue-state" v-if="!detail">
            {{
              mode === 'live'
                ? 'Select a flagged device to see its history and evidence.'
                : 'Select a device.'
            }}
          </div>
          <template v-else>
            <div class="entity-head">
              ...
              <div class="entity-badges">
                <span
                  class="badge strong"
                  :style="{
                    color: BAND_COLORS[detail.band],
                    borderColor: BAND_COLORS[detail.band],
                  }"
                >
            ...
            <div class="entity-section" v-if="chart">
              ...
              <p class="section-hint">
                One point per scored window. Lower is more anomalous. Raw isolation forest score,
                latest
              ...
              >
                ...
                <g v-for="row in chart.rows" :key="row.value">
                  <line
                    ...
                    stroke-width="1"
                  />
                  <text :x="CHART.pad.l - 8" :y="row.y + 3" text-anchor="end" class="axis-num">
                    {{ row.label }}
                  </text>
                </g>
                <line
                ...
                <path
                  :d="chart.line"
                  fill="none"
                  :stroke="BAND_COLORS[detail.band]"
                  stroke-width="1.8"
                  stroke-linejoin="round"
                />
                <circle
                  v-for="(point, index) in chart.flagged"
                  :key="index"
                  :cx="point.x"
                  :cy="point.y"
                  r="2.6"
            ...
            <div class="entity-section" v-if="detail.signals.length">
              ...
              <p class="section-hint">
                ...
                <template v-if="mode === 'sample'">
                  Peer median is the median across the other
                  {{ SAMPLE_STATUS.monitored - 1 }} monitored devices.
                </template>
                <template v-else>
                        ...
                        <span class="weight">
                          <svg viewBox="0 0 100 12" width="100" height="12" aria-hidden="true">
                            ...
                            <rect
                              x="0"
                              y="3"
                              :width="Math.min(100, signal.contribution * 100)"
                              height="6"
                              rx="3"
                              :fill="BAND_COLORS[detail.band]"
                            />
            ...
            <div class="entity-section">
              <h4>
                Evidence{{ detail.windowLabel ? `, flagged window ${detail.windowLabel}` : '' }}
              </h4>
              <p class="section-hint">
                The actual queries behind the score. This is the answer to "is it really dangerous",
                and it is the one thing the current page cannot show you.
              </p>
              ...
              <div
                class="evidence-state error"
                v-else-if="mode === 'live' && evidenceState === 'error'"
              >
                {{ evidenceError }}
              </div>
              <div
                ...
                v-else-if="
                  mode === 'live' &&
                  store.clientEvidence &&
                  !store.clientEvidence.window_within_retention &&
                  !detail.evidence.length
                "
              >
                This window is older than the {{ store.clientEvidence.retention_days }} day
                analytics retention, so the queries behind it have been pruned. The score survives
                for 30 days, the traffic does not.
              </div>
              ...
              <template v-else>
                <div class="evidence-summary" v-if="detail.evidenceSummary">
                  <span
                    ><b>{{ detail.evidenceSummary.total_queries.toLocaleString() }}</b>
                    queries</span
                  >
                  <span
                    ><b>{{ detail.evidenceSummary.distinct_domains.toLocaleString() }}</b> distinct
                    domains</span
                  >
                  <span
                    ><b>{{ detail.evidenceSummary.nxdomain_count.toLocaleString() }}</b>
                    NXDOMAIN</span
                  >
                  <span
                    ><b>{{ detail.evidenceSummary.blocked_count.toLocaleString() }}</b>
                    blocked</span
                  >
                </div>
                <div class="table-scroll">
                  <table class="evidence-table">
                    ...
                    <tbody>
                      <tr
                        v-for="(row, index) in detail.evidence"
                ...
                <p class="section-hint truncated" v-if="detail.evidenceTruncated">
                  Showing the top {{ detail.evidence.length }} groups. Raise the limit to see the
                  tail.
                </p>
              </template>
            </div>
            ...
            <div class="entity-actions">
              <button
                ...
                data-track="anomaly-preview-action"
                @click="notify('Open in DNS log')"
              >
                Open in DNS log
              </button>
              <button
              ...
              <button
                ...
                class="action"
                data-track="anomaly-preview-action"
                @click="notify('Mark benign, keep scoring')"
              >
                Mark benign, keep scoring
              </button>
              <button
                type="button"
                class="action"
                data-track="anomaly-preview-action"
                @click="notify('Whitelist device')"
              >
                Whitelist device
              </button>
              <button
...
const LEGEND = [
  {
    band: 'critical',
    title: 'Investigate',
    body: 'Off its own baseline and the traffic looks like tunneling, DGA or scanning.',
  },
  {
    band: 'suspicious',
    title: 'Suspicious shape, steady',
    body: 'Always does this. Either it is compromised and patient, or the shape is normal for it.',
  },
  {
    band: 'unusual',
    title: 'Just unusual',
    body: 'Changed its habits, but the traffic is ordinary. New app, new owner, firmware update.',
  },
  {
    band: 'quiet',
    title: 'Quiet',
    body: 'Within baseline. Shown so the map is a network view, not an alert list.',
  },
];
...
const rail = computed(() => {
  ...
  return {
    ...
    sidecarNote:
      daemon?.heartbeat_age_sec != null ? `heartbeat ${daemon.heartbeat_age_sec}s` : null,
...
const liveDetail = computed(() => {
  ...
  const history = [...store.clientHistory].sort((a, b) =>
  ...
  return {
    ...
    modelNote:
      store.clientModel?.training_rows != null
    ...
    evidence: (evidence?.rows || []).map((row) => ({
      ...
      type: row.query_type,
      response:
        row.action && row.action.startsWith('blocked') ? 'BLOCKED' : row.response_code || 'NOERROR',
      count: row.count,
    })),
...
const chart = computed(() => {
  ...
  const marks = [
    { x: chartX(0, values.length), label: `${values.length} windows ago`, anchor: 'start' },
  ];
  if (values.length > 4) {
    marks.push({
      x: chartX(Math.floor(values.length / 2), values.length),
      label: 'midpoint',
      anchor: 'middle',
    });
  }
...
const hasNext = computed(
  () => detailIndex.value >= 0 && detailIndex.value < queueRows.value.length - 1,
);
