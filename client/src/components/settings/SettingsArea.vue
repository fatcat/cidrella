  ...
  <div class="settings-area">
    <nav class="sa-rail" aria-label="Settings areas">
      ...
      <template v-for="group in visibleGroups" :key="group">
        ...
        <button
          v-for="a in matchingAreasInGroup(group)"
          :key="a.id"
          type="button"
          class="sa-rail-item"
          :class="{ active: a.id === areaId }"
          :aria-current="a.id === areaId ? 'page' : undefined"
          :data-track="a.dataTrack"
    ...
    <section class="sa-content" v-if="area">
      ...
      <div v-if="subtabs.length > 1" class="sa-subtabs" aria-label="Section">
        <button
          ...
          type="button"
          class="sa-subtab"
          :class="{ active: st.id === activeSecId }"
          :aria-current="st.id === activeSecId ? 'true' : undefined"
          :data-track="st.dataTrack"
          @click="$emit('sec', st.id)"
        >
          {{ st.label }}
        </button>
      </div>
