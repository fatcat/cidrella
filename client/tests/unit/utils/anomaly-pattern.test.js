...
describe('classifyClients', () => {
  // The stored score is negative when anomalous, so a worsening client's
  // score falls over time.
  it('flags a currently-active, falling score as escalating', () => {
    const events = [
      event('10.0.0.1', { daysAgo: 6, score: -0.1 }),
      event('10.0.0.1', { daysAgo: 3, score: -0.3 }),
      event('10.0.0.1', { daysAgo: 0, score: -0.6, resolved: 0 }),
    ];
