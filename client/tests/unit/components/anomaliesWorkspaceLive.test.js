...
describe('anomaly triage preview, live mode', () => {
  ...
  it('surfaces a pruned window instead of showing it as no traffic', async () => {
    api.get.mockImplementation((url) => {
      if (url === '/anomalies/summary')
        return Promise.resolve({ data: { enabled: true, daemon: null } });
      if (url.startsWith('/anomalies/events'))
        return Promise.resolve({ data: { events: [WORST], learning: [] } });
      if (url.includes('/evidence')) {
        return Promise.resolve({
          data: {
            retention_days: 7,
            window_within_retention: false,
            evidence_available: false,
            truncated: false,
            summary: null,
            rows: [],
            window_start: '2026-08-10 06:00:00',
            window_end: '2026-08-10 07:00:00',
          },
        });
      }
